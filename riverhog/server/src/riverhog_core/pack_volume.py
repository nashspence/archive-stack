...
def render_pack_upload_unit(
    plan: PackVolumePlan,
    unit_number: int,
    read_source_chunks: Callable[[str], Iterable[bytes]],
) -> bytes:
    return b"".join(iter_render_pack_upload_unit(plan, unit_number, read_source_chunks))
...
def render_pack_upload_unit_payload(
    plan: PackVolumePlan,
    unit_number: int,
    payload_chunks: Iterable[bytes],
) -> bytes:
    return b"".join(iter_render_pack_upload_unit_payload(plan, unit_number, payload_chunks))


def iter_pack_plaintext(
    plan: PackVolumePlan,
    read_source_chunks: Callable[[str], Iterable[bytes]],
) -> Iterator[bytes]:
    for unit in plan.units:
        yield from iter_render_pack_upload_unit(
            plan,
            unit.unit,
            read_source_chunks,
        )
...
def parse_pack_index(content: bytes | str) -> dict[str, object]:
    if isinstance(content, bytes):
        content = content.decode("utf-8")
    try:
        payload = json.loads(content)
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError("pack index is not valid JSON") from exc
    expected_fields = {"schema", "volume", "tree", "files"}
    if (
        not isinstance(payload, dict)
        or payload.get("schema") != PACK_INDEX_SCHEMA
        or set(payload) != expected_fields
    ):
        raise ValueError("pack index schema mismatch")
    volume = payload.get("volume")
    tree = payload.get("tree")
    files = payload.get("files")
    if (
        not isinstance(volume, dict)
        or set(volume) != {"id", "sequence"}
        or not isinstance(tree, dict)
        or set(tree) != {"files", "bytes", "sha256"}
        or not isinstance(files, list)
    ):
        raise ValueError("pack index structure is invalid")
    volume_id = str(volume.get("id", ""))
    sequence = _canonical_nonnegative_int(volume.get("sequence"), label="pack sequence")
    if volume_id != f"pack-{sequence:012d}":
        raise ValueError("pack index volume identity is invalid")
    ...
    for raw in files:
        ...
        if not isinstance(raw, dict) or set(raw) != expected_file_fields:
            raise ValueError("pack index file must be a canonical mapping")
        path = normalize_relpath(str(raw.get("path", "")))
        if path.startswith(RESERVED_ARCHIVE_PREFIX) or path in seen:
            raise ValueError("pack index file path is invalid")
        byte_count = _canonical_nonnegative_int(raw.get("bytes"), label="file bytes")
        sha256 = str(raw.get("sha256", ""))
        unit = _canonical_nonnegative_int(raw.get("unit"), label="file unit")
        header_offset = _canonical_nonnegative_int(
            raw.get("header_offset"), label="file header offset"
        )
        data_offset = _canonical_nonnegative_int(raw.get("data_offset"), label="file data offset")
        if _SHA256_RE.fullmatch(sha256) is None:
            raise ValueError("pack index file sha256 is invalid")
        expected_data_offset = header_offset + len(_tar_header(path, byte_count))
        if (
            ...
            or (unit == previous_unit and header_offset != expected_header_offset)
        ):
            raise ValueError("pack index file offsets or unit order are invalid")
        previous_unit = unit
        expected_header_offset = data_offset + byte_count + _tar_padding(byte_count)
        seen.add(path)
    ...
    if not rows:
        raise ValueError("pack index files must not be empty")
    if _canonical_nonnegative_int(tree.get("files"), label="tree files") != len(rows):
        raise ValueError("pack index tree file count mismatch")
    if _canonical_nonnegative_int(tree.get("bytes"), label="tree bytes") != total_bytes:
        raise ValueError("pack index tree byte count mismatch")
    if str(tree.get("sha256", "")) != tree_digest.hexdigest():
        raise ValueError("pack index tree sha256 mismatch")
    return {
        "schema": PACK_INDEX_SCHEMA,
        "volume": {"id": volume_id, "sequence": sequence},
        "tree": {"files": len(rows), "bytes": total_bytes, "sha256": tree_digest.hexdigest()},
        "files": rows,
    }
