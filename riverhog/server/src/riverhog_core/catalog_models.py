...
class CollectionArchiveCopyRecord(Base):
    ...
    storage_adapter: Mapped[str | None] = mapped_column(String, nullable=True)
    storage_profile_id: Mapped[str | None] = mapped_column(String, nullable=True)
    storage_profile_contract_sha256: Mapped[str | None] = mapped_column(
        String(64), nullable=True
    )
    egress_accounting_id: Mapped[str | None] = mapped_column(String, nullable=True)
    read_mode: Mapped[str | None] = mapped_column(String, nullable=True)
    adapter_implementation_id: Mapped[str | None] = mapped_column(String, nullable=True)
    adapter_implementation_version: Mapped[str | None] = mapped_column(
        String, nullable=True
    )
    adapter_source_revision: Mapped[str | None] = mapped_column(String, nullable=True)
    adapter_runtime_descriptor_sha256: Mapped[str | None] = mapped_column(
        String(64), nullable=True
    )
...
class ArchiveCopyJobRecord(Base):
    ...
    storage_adapter_runtime_descriptor_sha256: Mapped[str] = mapped_column(String(64))
...
class ArchiveCopyObjectUploadRecord(Base):
    ...
    plaintext_bytes: Mapped[int] = mapped_column(BigInteger)
    sha256: Mapped[str | None] = mapped_column(String(64), nullable=True)
    multipart_transfer_id: Mapped[str | None] = mapped_column(String, nullable=True)
    adapter_runtime_descriptor_sha256: Mapped[str | None] = mapped_column(
        String(64), nullable=True
    )
