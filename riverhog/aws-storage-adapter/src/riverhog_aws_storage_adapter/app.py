"""AWS storage-adapter process entrypoint."""
...
def build_service(config: AwsStorageAdapterConfig) -> StorageAdapterService:
    return StorageAdapterService(
        driver=AwsStorageDriver(
            config,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--host",
        default=os.getenv("RIVERHOG_AWS_STORAGE_ADAPTER_HOST", "127.0.0.1"),
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("RIVERHOG_AWS_STORAGE_ADAPTER_PORT", "8080")),
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = AwsStorageAdapterConfig.from_env()
