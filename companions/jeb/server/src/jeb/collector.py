...
class Collector:
    ...
    def consume_munchy_events_forever(self) -> None:
        ...
        with MunchyClient(target.url, token=target.token) as client:
            while True:
                try:
    ...
    def consume_munchy_events_once(self, client: MunchyClient) -> int:
    ...
    def _apply_source_removal(self, plan: Mapping[str, Any]) -> None:
        ...
        if bool(plan["purge"]):
            for attempt in cast(list[dict[str, Any]], plan["active_attempts"]):
                ...
                adapter.cancel(self, str(attempt["id"]))
    ...
    def _process_attempt_locked(self, attempt_id: str) -> None:
        try:
            ...
            adapter = self.target_adapters["munchy"]
            adapter.advance(self, attempt_id)
