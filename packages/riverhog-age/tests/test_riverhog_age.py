...
def test_multipart_part_plans_reconstruct_exact_age_file():
    ...
    plans = session.multipart_part_plans(len(plaintext), chunks_per_part=3)
    assert len(plans) == 3
...
def test_resume_from_middle_missing_part():
    ...
    plans = original.multipart_part_plans(len(plaintext), chunks_per_part=2)
...
def test_default_multipart_plan_handles_70_gib_archive_shape():
    session = new_test_session()
    seventy_gib = 70 * 1024**3
    plans = session.multipart_part_plans(seventy_gib)
    assert len(plans) == 1_120
    assert all(current.ciphertext_len > 0 for current in plans)
    assert plans[-1].ciphertext_end == age_ciphertext_len_for_plaintext_len(
        seventy_gib, age_prefix_len=len(session.age_prefix)
    )


def test_multipart_plan_requires_positive_chunk_grouping():
    session = new_test_session()
    with pytest.raises(ValueError, match="positive"):
        session.multipart_part_plans(CHUNK_SIZE, chunks_per_part=0)


def test_multipart_plan_has_no_provider_specific_member_ceiling():
    session = new_test_session()
    count = 10_001
    plans = session.multipart_part_plans(count * CHUNK_SIZE, chunks_per_part=1)
...
def test_ranged_plaintext_provider_reconstructs_exact_age_file():
    ...
    session = new_test_session(len(plaintext))
    full = session.encrypt_plaintext(plaintext)
    plans = session.multipart_part_plans(len(plaintext), chunks_per_part=2)
