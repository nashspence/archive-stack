...
def file_is_fully_compliant(
    ...
    *,
    ...
    if not image_ids:
        return False
    required_by_image = {
        image.image_id: normalize_required_disc_count(image.required_disc_count)
    ...
    for disc in session.scalars(
        select(ImageDiscRecord).where(ImageDiscRecord.image_id.in_(image_ids))
    ).all():
        if disc_counts_as_verified(
            state=disc.state,
            verification_state=disc.verification_state,
        ):
            verified_counts[disc.image_id] = verified_counts.get(disc.image_id, 0) + 1
    return all(
