...
APPLICATION_PERMISSIONS = frozenset(
    {SUBMISSIONS_MANAGE, EVENTS_READ, EVENTS_READ_ALL}
)
...
def _validate_list(
    *, page: int, per_page: int, sort: str, order: str, fields: set[str]
) -> None:
    if page < 1:
...
def _page_metadata(
    *, page: int, per_page: int, total: int, all_items: bool
) -> dict[str, int]:
    return {
...
class SQLiteApplicationKeyStore:
    ...
    def list_apps(
        ...
        *,
        ...
        return {
            **_page_metadata(
                page=page, per_page=per_page, total=total, all_items=all_items
            ),
            "sort": sort,
            "order": order,
            "query": query,
