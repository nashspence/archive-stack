...
def create_template_registry_snapshot(source_db: Path, snapshot_db: Path) -> int:
    """Atomically create a verified, template-only snapshot from a live runner DB."""
    ...
    try:
        ...
        with closing(sqlite3.connect(pending_snapshot)) as snapshot:
            ...
            snapshot.execute("ATTACH DATABASE ? AS runner", (str(online_copy),))
            columns = ", ".join(JOB_TEMPLATE_COLUMNS)
            snapshot.execute(
                f"INSERT INTO main.job_templates({columns}) "
                f"SELECT {columns} FROM runner.job_templates ORDER BY name"
            )
            snapshot.commit()
            snapshot.execute("DETACH DATABASE runner")
...
def restore_template_registry_snapshot(
    ...
    *,
    ...
    with closing(sqlite3.connect(target_db, timeout=30)) as target:
        ...
        _quick_check(target, label="restored runner database")
...
def _parser() -> argparse.ArgumentParser:
    ...
    restore = subparsers.add_parser("restore", help="restore templates into a runner database")
