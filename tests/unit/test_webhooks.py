...
def test_build_images_ready_payload_supports_multiple_images() -> None:
    payload = build_images_ready_payload(
        ...
        batch=ImagesReadyBatch(
            batch_id='batch-1',
            images=[
                ReadyImage(image_id='img1', filename='img1.iso', iso_available=True),
                ReadyImage(image_id='img2', filename='img2.iso', iso_available=True),
            ],
        ),
    ...
    assert payload['images'][0]['download_url'].endswith('/v1/images/img1/iso')
