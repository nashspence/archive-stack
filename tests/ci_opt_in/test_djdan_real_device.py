...
from djdan import main as djdan_main
...
def _reject_fake_factories() -> None:
    ...
    if configured:
        ...
        pytest.fail(
            f"ci-opt-in djdan validation must not use fake factory env vars: {names}"
        )
...
def _require_destructive_opt_in(*, reason: str) -> None:
    if os.environ.get("DJDAN_CI_OPT_IN_BURN_CONFIRM") == _BURN_CONFIRMATION:
        return
    pytest.skip(
        "set DJDAN_CI_OPT_IN_BURN_CONFIRM=write-optical-media to run destructive "
        f"{reason}"
    )


def _required_burn_device(*, reason: str) -> Path:
    device = _required_path("DJDAN_CI_OPT_IN_BURN_DEVICE", reason=reason)
    if not os.access(device, os.R_OK | os.W_OK):
        pytest.skip(
            f"DJDAN_CI_OPT_IN_BURN_DEVICE must be readable and writable: {device}"
        )
    return device
...
def _run_djdan(
    ...
    *args: str,
    ...
    env["RIVERHOG_BASE_URL"] = acceptance_system.base_url
    if staging_dir is not None:
        env["DJDAN_STAGING_DIR"] = str(staging_dir)
...
def _assert_djdan_succeeded(result: subprocess.CompletedProcess[str]) -> None:
    assert result.returncode == 0, (
        "djdan command failed\n"
        f"stdout:\n{result.stdout}\n"
        f"stderr:\n{result.stderr}"
    )
...
def _sha256_chunks(chunks: Iterator[bytes]) -> str:
    ...
    for chunk in chunks:
        ...
        digest.update(chunk)
    if not saw_bytes:
        raise AssertionError("djdan optical reader returned no bytes")
...
def _read_recovery_copy_sha256(
    ...
    *,
    ...
    copy = djdan_main.RecoveryCopyHint(
        copy_id="ci-opt-in-djdan-copy",
        location="ci-opt-in validation media",
        disc_path=disc_path,
        recovery_bytes=0,
        recovery_sha256="",
    )
    return _sha256_chunks(djdan_main._iter_recovered_chunks(reader, copy, device=device))


def test_default_reader_recovers_configured_mounted_media_payload() -> None:
    _reject_fake_factories()
    mount_path = _required_path(
        "DJDAN_CI_OPT_IN_MOUNT_PATH",
        reason="mounted optical-media recovery validation",
    )
    if not mount_path.is_dir():
        pytest.skip(
            f"DJDAN_CI_OPT_IN_MOUNT_PATH must be a mounted directory: {mount_path}"
        )
    disc_path = _required_env(
        "DJDAN_CI_OPT_IN_PAYLOAD_PATH",
        reason="mounted optical-media recovery validation",
    )
    expected_sha256 = _required_env(
        "DJDAN_CI_OPT_IN_EXPECTED_SHA256",
        reason="mounted optical-media recovery validation",
    )

    reader = djdan_main.build_optical_reader()
    if not isinstance(reader, djdan_main.XorrisoOpticalReader):
...
def test_default_reader_recovers_configured_raw_device_payload() -> None:
    ...
    device = _required_path(
        "DJDAN_CI_OPT_IN_RAW_DEVICE",
        reason="raw optical-device recovery validation",
    )
    if not os.access(device, os.R_OK):
        pytest.skip(f"DJDAN_CI_OPT_IN_RAW_DEVICE is not readable by this user: {device}")
    disc_path = _optional_env(
        "DJDAN_CI_OPT_IN_RAW_PAYLOAD_PATH",
        "DJDAN_CI_OPT_IN_PAYLOAD_PATH",
        reason="raw optical-device recovery validation",
    )
    expected_sha256 = _optional_env(
        "DJDAN_CI_OPT_IN_RAW_EXPECTED_SHA256",
        "DJDAN_CI_OPT_IN_EXPECTED_SHA256",
        reason="raw optical-device recovery validation",
    )
...
def test_destructive_burn_backend_writes_and_verifies_configured_media(tmp_path: Path) -> None:
    ...
    copy_id = os.environ.get("DJDAN_CI_OPT_IN_BURN_COPY_ID", "ci-opt-in-djdan-copy")

    iso_verifier = djdan_main.build_iso_verifier()
    if not isinstance(iso_verifier, djdan_main.XorrisoIsoVerifier):
        raise AssertionError(
            f"default ISO verifier is not the real backend: {type(iso_verifier).__name__}"
        )
    burner = djdan_main.build_disc_burner()
    if not isinstance(burner, djdan_main.XorrisoDiscBurner):
        raise AssertionError(f"default burner is not the real backend: {type(burner).__name__}")
    verifier = djdan_main.build_burned_media_verifier()
    if not isinstance(verifier, djdan_main.RawBurnedMediaVerifier):
        raise AssertionError(
...
def test_full_burn_then_fetch_cli_workflow_uses_fake_api_and_real_optical_device(
    ...
    _reject_fake_factories()
    _require_xorriso(reason="full djdan burn/fetch CLI workflow validation")
    _require_destructive_opt_in(reason="full djdan burn/fetch CLI workflow validation")
    device = _required_burn_device(reason="full djdan burn/fetch CLI workflow validation")
    ...
    staging_dir = tmp_path / "djdan-burn-fetch-staging"
    ...
    _assert_djdan_succeeded(result)
...
def test_full_recover_cli_workflow_uses_fake_api_restored_iso_and_real_optical_device(
    ...
    _reject_fake_factories()
    _require_xorriso(reason="full djdan recover CLI workflow validation")
    _require_destructive_opt_in(reason="full djdan recover CLI workflow validation")
    device = _required_burn_device(reason="full djdan recover CLI workflow validation")
    ci_opt_in_acceptance_system.enable_real_iso_streams()
    _image_id, session_id, burned_copy_id = _prepare_recovery_session_with_one_real_burn_needed(
        ci_opt_in_acceptance_system
    )
    staging_dir = tmp_path / "djdan-recover-staging"
