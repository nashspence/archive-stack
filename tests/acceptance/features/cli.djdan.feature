...
Feature: djdan CLI
  ...
  Background:
    Given split disc-covered target "docs/tax/2022/invoice-123.pdf" has queued fetch "fx-1"
  ...
  Scenario: djdan fetch completes a recoverable fetch
    ...
    And stderr mentions disc id "20260420T040003Z-1"
    And stderr mentions disc id "20260420T040004Z-1"
  ...
  Scenario: djdan fetch resumes split recovery across repeated runs via server-side upload state
    Given the configured optical reader cannot recover disc id "20260420T040004Z-1"
    When the operator runs djdan fetch "fx-1"
    Then the command exits non-zero
    And fetch "fx-1" is not "done"
    When the configured optical reader cannot recover disc id "20260420T040003Z-1"
    And the operator runs djdan fetch "fx-1" with JSON output
    Then the command exits with code 0
    And stdout is valid JSON
    And stdout reports fetch state "done"
    And stderr does not mention disc id "20260420T040003Z-1"
    And stderr mentions disc id "20260420T040004Z-1"
    And target for fetch "fx-1" is hot
  ...
  Scenario: djdan fetch fails if the server rejects incorrect recovered bytes
    ...
    And stderr mentions "try another registered disc or recovered media"
  ...
  Scenario: djdan image list prints finalized image records
    ...
    And stdout mentions "20260420T040001Z"
    And stdout mentions "redundancy:"
