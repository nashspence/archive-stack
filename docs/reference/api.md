...
- finalized-image ISO responses include `Accept-Ranges: bytes`
- finalized-image ISO responses support a single HTTP byte range and return `206 Partial Content` with
  `Content-Range` when a valid `Range: bytes=...` request is supplied
